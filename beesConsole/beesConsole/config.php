...
function addroom_url($rid){
	return 'http://192.168.3.108:8080/grapes/addroom?rid='.$rid;
}
//删除文件
function delroom_url($rid){
	return 'http://192.168.3.108:8080/grapes/delroom?rid='.$rid;
}
//发送消息
function sendmsg_url($rid,$msg,$repeat='1',$color='0'){
	return 'http://192.168.3.108:8080/grapes/testmsg?rid='.$rid.'&msg='.$msg.'&repeat='.$repeat.'&col='.$color;
}
//设置房间参数
function setroom_url($rid,$num,$interval,$pause,$freq,$a0,$a1,$a2,$a3){
	return 'http://192.168.3.108:8080/grapes/setroom?rid='.$rid.'&num='.$num.'&interval='.$interval.'&pause='.$pause.'&freq='.$freq.'&a0='.$a0.'&a1='.$a1.'&a2='.$a2.'&a3='.$a3;
}
//查看房间参数
function roominfo_url($rid){
    return 'http://192.168.3.108:8080/grapes/roominfo?rid='.$rid;
}
//查看房间参数
function roominfo_All(){
    return 'http://192.168.3.108:8080/grapes/roominfo';
}
//获取房间的帐号类型
function ctypeinfo_url($rid){
    return 'http://192.168.3.197:8000/ctypeinfo?rid='.$rid;
}