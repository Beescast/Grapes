...
?>
<div class="btn-group">
	<a href="<?php echo site_urlc('room/create')?>" class='btn btn-primary'> <i class="fa fa-plus"></i> <?php echo $title; ?> </a>
</div>
...
<table class="table table-striped table-hover select-list">
	...
	<tbody class="sort-list">
		...
		<tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
			...
			<td> <input type="text" class="sortid" value="<?php echo $v['sort_id']?>" data-id="<?php echo $v['id'] ?>"> <a href="https://www.douyu.com/<?php echo $v['title'] ?>" target="_blank" ><?php echo $v['title'] ?></a></td>
            <td><?php 
			echo getRoomStatus($v['title']);
			?></td>
			<td><?php echo $v['nickname']?></td>
			<td><?php echo $v['AccountNumber']?></td>
			...
			<?php if (!$this->ccid): ?>
			<td>
			<a href="<?php echo site_url('room/index?c=5&cc='.$v['id']) ?>"> 伪子栏目文章 </a>
			...
			<td><a href="#info" role="button" class="btn btn-info" data-toggle="modal"  onclick="getRoomInfo('<?php echo $v['title']?>');">详细信息</a></td>
			<td>
				<div class="btn-group">
...
<div class="btn-group">
	...
	<a id="btn-audit" class='btn' href="#" data-audit='1'>开启</a>
	<a id="btn-audit" class='btn' href="#"  data-audit='0'>关闭</a>
</div>
